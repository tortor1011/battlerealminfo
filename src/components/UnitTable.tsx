    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-zinc-600 transition-all duration-200 hover:shadow-lg hover:shadow-black/30">
      ...
      <div className="flex items-start justify-between gap-3 p-4 pb-3 border-b border-zinc-800/80">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {unit.isHealer && (
              <span className="text-[10px] font-bold bg-rose-900/50 text-rose-300 border border-rose-700/50 rounded-full px-2 py-0.5 flex items-center gap-1">
                <Heart className="w-2.5 h-2.5" /> {t(UI.unitTable.healerTag)}
              </span>
            )}
          </div>
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-zinc-500">{t(unit.role)}</span>
          </div>
        </div>
      ...
      <div className="p-4 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1.5">
              {unit.battleGears.map((gear) => (
                <div key={gear.name} className="bg-amber-950/30 border border-amber-800/40 rounded-xl p-2.5">
                  ...
                  <p className="text-[11px] text-zinc-400 leading-relaxed">{t(gear.description)}</p>
                  <div className="flex items-center gap-1 mt-1.5">
                    <Sword className="w-2.5 h-2.5 text-zinc-600" />
                    <span className="text-[10px] text-zinc-600">
                      {t(UI.unitTable.requires)} {gear.buildingRequired}
                    </span>
                  </div>
                </div>
              ))}
        ...
        <div className="flex items-start gap-2 bg-zinc-800/40 rounded-xl px-3 py-2.5">
          ...
          <p className="text-[11px] text-zinc-400 leading-relaxed">{t(unit.notes)}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        <h2 className="text-lg font-bold text-zinc-200">{t(UI.unitTable.selectClan)}</h2>
      ...
      {/* Clan Lore Banner */}
      <div className={`bg-gradient-to-r ${activeClan.color} rounded-2xl p-5 border`}>
        <h3 className="text-base font-bold text-zinc-100 mb-1">{activeClan.name}</h3>
        <p className="text-sm text-zinc-300 leading-relaxed">{t(activeClan.lore)}</p>
      </div>
      ...
      {activeClan.units.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-4 bg-zinc-900 border border-zinc-800 border-dashed rounded-2xl">
          ...
          <div className="text-center">
            ...
            <p className="text-zinc-600 text-sm mt-1">{t(UI.unitTable.comingSoonSub)}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-xs text-zinc-500 ml-6">{t(UI.unitTable.damageMatchupDesc)}</p>
        </div>
